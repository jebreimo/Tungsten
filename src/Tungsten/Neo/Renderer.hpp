...
namespace Tungsten
{
    ...
    // The render side (§1, §5): draws a RenderSnapshot, never touching the
    // scene graph — only the immutable snapshot and the resources its refs
    // name (fetched through the ResourceManager).
    //
    // It owns the three UBO buffer objects of the fixed binding convention
    // (§4). The binding points are set once, in the constructor; afterwards
