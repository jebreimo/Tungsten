...
{
    /**
     * The frame-tagged deferred-deletion machinery, owned by ResourceManager
     * and shared by every subsystem that takes a GL object out of service:
     * an arena's displaced BufferHandle after a grow, a destroyed resource's
     * ProgramHandle / TextureHandle, and a VertexArrayHandle evicted
     * from the VaoCache.
     *
     * A GL object is not deleted the moment it is retired, because an
     * in-flight frame (single-threaded: the frame being drawn; threaded:
     * a snapshot behind a fence) may still reference it. Instead its owning
     * RAII handle is moved onto the queue tagged with the current frame, and
     * freed only once a completed frame proves nothing references it.
     */
    class DeletionQueue
    {
    public:
