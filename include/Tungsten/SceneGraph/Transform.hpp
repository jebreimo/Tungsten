...
namespace Tungsten
{
     ...
     * where forward is -z, up is +y and right is +x. A 120 degree turn about
     * (-1, 1, 1); every component is exactly one half.
     *
     * Mount a node on a vehicle whose orientation comes from a physics or
     * flight model in those axes and it will otherwise face sideways — most
     * visibly for a camera, which looks along its node's -z.
     */
    [[nodiscard]]
    ...
    /**
     * Returns @a rotation, an orientation expressed in the +x forward, +y left,
     * +z up frame, expressed in the scene graph's frame instead. Both the
     * reference and the rotated axes are rebased, hence the conjugation rather
     * than a plain multiplication.
     */
    [[nodiscard]]
